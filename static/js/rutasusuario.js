...
function setupFormSubmitHandler() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            console.log('selectedCanjes:', selectedCanjes);
            
            // Validar que haya canjes seleccionados
            if (selectedCanjes.length === 0) {
                console.warn('ADVERTENCIA: No hay canjes seleccionados');
            }
            
            // Agregar el campo materiales_seleccionados antes de enviar
            let hiddenMateriales = document.getElementById('hidden_materiales_seleccionados');
            if (!hiddenMateriales) {
                hiddenMateriales = document.createElement('input');
                hiddenMateriales.type = 'hidden';
                hiddenMateriales.id = 'hidden_materiales_seleccionados';
                hiddenMateriales.name = 'materiales_seleccionados';
                form.appendChild(hiddenMateriales);
            }
            
            // Convertir selectedCanjes a JSON con el formato esperado
            const materialesData = selectedCanjes.map(canje => ({
                tipo: 'canje',
                id: canje.id,
                material: canje.material,
                peso: canje.peso,
                puntos: canje.puntos
            }));
            
            hiddenMateriales.value = JSON.stringify(materialesData);
            console.log('Materiales JSON:', hiddenMateriales.value);
            console.log('Total canjes:', materialesData.length);
        });
    } else {
        console.error('ERROR: No se encontró el formulario #nuevaRecoleccionForm');
    }
}
