...
# Debug view: listar archivos en media/fotos_perfil (solo staff)
@staff_member_required
def debug_media_list(request):
    """Vista temporal para administradores: lista archivos dentro de media/fotos_perfil/ usando default_storage.
    Útil para confirmar si las subidas se escriben en el filesystem o si se están almacenando en S3/Spaces.
    """
    prefix = 'fotos_perfil'
    files_info = []
    error = None
    try:
        ...
        # storage.listdir devuelve (dirs, files)
        dirs, files = storage.listdir(prefix)
        for fname in files:
            rel_path = os.path.join(prefix, fname)
            try:
    ...
    return render(request, 'core/debug_media_list.html', {
        'prefix': prefix,
        'files': files_info,
        'error': error,
    })
...
def healthz(request):
    """Health check endpoint. Devuelve 200 rápidamente sin acceder a la base de datos.

    Útil para que el balanceador / Railway verifique que el proceso Django/Daphne
    está en ejecución. No realiza operaciones de base de datos ni llamadas a servicios
    externos para evitar falsos negativos durante el arranque.
    """
...
# Supabase integration removed — using PostgreSQL (Django ORM) only.
...
def registrate(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Verificar si EMAIL está configurado correctamente
            from django.conf import settings
            email_configurado = bool(settings.EMAIL_HOST_USER and settings.EMAIL_HOST_PASSWORD)
            
            if not email_configurado:
                # Sin configuración de email - activar usuario directamente
                user.is_active = True
                user.email_verificado = True
                user.save()
                
                # Otorgar logro de registro
            ...
            try:
                ...
                if success:
                    ...
                    # Si falla el envío del email, activar usuario directamente como fallback
                    print(f"⚠️ Fallo envío de email: {message}. Activando usuario sin verificación.")
                    user.is_active = True
                    user.email_verificado = True
                    user.save()
                ...
                print(f"❌ Error crítico enviando email: {email_error}")
                user.is_active = True
                user.email_verificado = True
                user.save()
...
def perfil(request):
    ...
    if request.method == 'POST':
        if 'photo_submit' in request.POST:
            ...
            if 'foto_perfil' in request.FILES:
                from django.utils import timezone
                try:
                    uploaded = request.FILES['foto_perfil']
                    # asignar y guardar
                    user.foto_perfil = uploaded
                    ...
                    # Registrar y notificar error al usuario
                    print(f"Error al guardar foto de perfil: {ex}")
                    messages.error(request, 'No se pudo guardar la foto. Intenta con otra imagen o verifica permisos del servidor.')
                    return redirect('perfil')
    ...
    # Verificar que la imagen de perfil exista en el storage
    foto_perfil_missing = False
    try:
        if getattr(user, 'foto_perfil', None):
            name = getattr(user.foto_perfil, 'name', None)
            if name:
                try:
                    exists = default_storage.exists(name)
                except Exception as _e:
                    # Si el storage lanza excepción, no romper la vista en producción
                    print(f"Warning checking storage for foto_perfil (storage.exists failed): {_e}")
                    exists = False

                if not exists:
                    foto_perfil_missing = True
                    # Añadir mensaje para depuración/usuario (solo una vez)
                    try:
                        messages.warning(request, 'No se encontró la imagen de perfil en el servidor. Si la subiste recientemente, intenta recargar o contactar al administrador.')
                    except Exception:
                        # No bloquear la vista si el sistema de mensajes falla
                        print('Warning: messages system failed while warning about missing profile image')
    except Exception as e:
        # Capturar cualquier error inesperado durante la comprobación y evitar 500
        print(f"Error inesperado comprobando foto_perfil: {e}")
        foto_perfil_missing = False
    ...
    context = {
        ...
        'user': user,
        'foto_perfil_missing': foto_perfil_missing,
...
def terminar_sesion(request, session_id):
    ...
    try:
        from .security import SecurityManager
        ok, msg = SecurityManager.force_logout_session(session_id)
        if ok:
...
def cerrar_sesion_admin(request, session_id):
    ...
    try:
        ...
        # Usar el SecurityManager para forzar logout y notificar al cliente
        from .security import SecurityManager
        ok, message = SecurityManager.force_logout_session(session_id)

        # Además intentar invalidar sesiones de Django asociadas
        try:
...
def verificar_sesion_activa(request):
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        try:
            ...
            # Si hay un token en la sesión del navegador, verificar específicamente esa sesión
            if session_token:
                try:
                    ses = SesionUsuario.objects.get(token_sesion=session_token, usuario=request.user)
                    if ses.activa and ses.fecha_expiracion > timezone.now():
                        logger.info(f"Sesión específica válida para {request.user.username}")
                        return JsonResponse({'activa': True, 'message': 'Sesión activa'})
                    else:
                        logger.warning(f"Sesión específica inválida/expirada para {request.user.username}")
                        return JsonResponse({'activa': False, 'message': 'Tu sesión ha sido cerrada por un administrador.'})
                except SesionUsuario.DoesNotExist:
                    logger.warning(f"Token de sesión no encontrado en BD para {request.user.username}")
                    return JsonResponse({'activa': False, 'message': 'Tu sesión ha sido cerrada por un administrador.'})

            # Si no hay token en la sesión, buscar TODAS las sesiones activas para este usuario
            sesiones_activas = SesionUsuario.objects.filter(usuario=request.user, activa=True)
            ...
            # Si hay sesiones activas, actualizar token en session y devolver activo
            valid = None
            ...
            if valid:
                request.session['session_token'] = valid.token_sesion
                logger.info("Token actualizado en sesión de Django")
                return JsonResponse({'activa': True, 'message': 'Sesión activa'})

            # Si llegamos acá, no hay sesiones válidas
            logger.info(f"Todas las sesiones de {request.user.username} están expiradas")
            return JsonResponse({'activa': False, 'message': 'Tu sesión ha expirado.'})
...
@login_required
@user_passes_test(lambda u: u.role == 'admin')
def asignar_ruta_conductor(request, conductor_id):
    """
    Vista para abrir un modal con el mapa de Ibagué y asignar ruta a un conductor
    """
    try:
        conductor = get_object_or_404(Usuario, id=conductor_id, role='conductor')
        
        # Obtener rutas disponibles
        rutas = RutaRecoleccion.objects.filter(
            estado='planificada'
        ).order_by('nombre')
        ...
        return render(request, 'core/modales/modal_asignar_ruta.html', context)
    except Usuario.DoesNotExist:
        return JsonResponse({
            'success': False,
...
@login_required
@user_passes_test(lambda u: u.role == 'admin')
def get_rutas_disponibles(request, conductor_id):
    """
    API AJAX para obtener rutas disponibles para un conductor
    """
    try:
        conductor = get_object_or_404(Usuario, id=conductor_id, role='conductor')
        ...
        rutas = RutaRecoleccion.objects.filter(
            conductor__isnull=True  # Sin conductor asignado aún
        ).values('id', 'nombre', 'zona', 'fecha_programada')
...
@user_passes_test(lambda u: u.role == 'admin')
@require_POST
def api_asignar_ruta_conductor(request, conductor_id):
    """
    API para asignar una ruta a un conductor
    """
    try:
        conductor = get_object_or_404(Usuario, id=conductor_id, role='conductor')
        ...
        if not ruta_id:
            return JsonResponse({
                'success': False,
                'message': 'ID de ruta es requerido'
            }, status=400)
        
        # Obtener la ruta
        ruta = RutaRecoleccion.objects.get(id=ruta_id)
        
        # Asignar conductor a la ruta
        ruta.conductor = conductor
        ruta.estado = 'programada'  # Cambiar estado a programada
        ruta.save()
        
        # Actualizar el campo ruta_asignada del usuario (si existe)
        if hasattr(conductor, 'ruta_asignada'):
            conductor.ruta_asignada = ruta
            conductor.save()
        ...
        Notificacion.objects.create(
            ...
            mensaje=f'Se te ha asignado la ruta: {ruta.nombre}. Zona: {ruta.zona}. Fecha: {ruta.fecha_programada}',
            tipo='ruta_asignada'
        )
        
        # Enviar correo al conductor
        try:
            subject = f'🚗 Nueva Ruta Asignada - {ruta.nombre}'
            ...
            <head>
                ...
                <style>
                    ...
                    .container {{ max-width: 600px; margin: 0 auto; padding: 20px; }}
                    .header {{ background: linear-gradient(135deg, #3498db 0%, #2980b9 100%); color: white; padding: 30px; text-align: center; border-radius: 15px 15px 0 0; }}
                    .content {{ background: #ecf0f1; padding: 30px; border-radius: 0 0 15px 15px; }}
                    .card {{ background: white; padding: 25px; border-radius: 10px; margin: 20px 0; box-shadow: 0 3px 10px rgba(0,0,0,0.1); border-left: 5px solid #3498db; }}
                    ...
                    .value {{ color: #3498db; font-weight: 600; }}
                    .btn {{ background: linear-gradient(135deg, #3498db 0%, #2980b9 100%); color: white; padding: 12px 30px; text-decoration: none; border-radius: 25px; display: inline-block; margin-top: 20px; font-weight: 600; }}
                    .footer {{ text-align: center; margin-top: 30px; color: #7f8c8d; font-size: 12px; }}
                    .badge {{ display: inline-block; background: #27ae60; color: white; padding: 5px 10px; border-radius: 20px; margin: 5px; font-size: 12px; }}
                </style>
            </head>
            <body>
                <div class="container">
                    <div class="header">
                        ...
                        <p>¡Tienes una nueva misión de recolección!</p>
                    </div>
                    <div class="content">
                        <p>Hola <strong>{conductor.first_name or conductor.username}</strong>,</p>
                        <p>¡Excelente! Se te ha asignado una nueva ruta de recolección. A continuación encontrarás todos los detalles:</p>
                        
                        <div class="card">
                            <h3 style="color: #3498db; margin-top: 0;">📍 Detalles de la Ruta</h3>
                            <div class="info-row">
                                <span class="label">🎯 Nombre de Ruta:</span>
                                <span class="value">{ruta.nombre}</span>
                            </div>
                            <div class="info-row">
                                <span class="label">📍 Zona:</span>
                                <span class="value">{ruta.zona} - {ruta.zona.capitalize()}</span>
                            </div>
                            <div class="info-row">
                                <span class="label">📅 Fecha Programada:</span>
                                <span class="value">{ruta.fecha_programada}</span>
                            </div>
                            <div class="info-row">
                                <span class="label">🕐 Hora Inicio:</span>
                                <span class="value">{ruta.hora_inicio if hasattr(ruta, 'hora_inicio') else 'Por confirmar'}</span>
                            </div>
                            <div class="info-row">
                                <span class="label">🚚 Tipo de Vehículo:</span>
                                <span class="value">{ruta.vehiculo_tipo if hasattr(ruta, 'vehiculo_tipo') else 'Estándar'}</span>
                            </div>
                        </div>
                        
                        <div class="card" style="border-left-color: #27ae60;">
                            <h3 style="color: #27ae60; margin-top: 0;">✅ Instrucciones Importantes</h3>
                            <ul style="padding-left: 20px; line-height: 1.8;">
                                <li>Asegúrate de revisar la ruta completa antes de la fecha programada</li>
                                <li>Verifica tu vehículo y equipo de seguridad</li>
                                <li>Llega 15 minutos antes de la hora de inicio</li>
                                <li>Comunícate con el administrador si hay cambios o inconvenientes</li>
                                <li>Actualiza el estado de la ruta durante la ejecución</li>
                            </ul>
                        </div>
                        
                        <div style="text-align: center;">
                            <a href="http://127.0.0.1:8000/panel_conductor/" class="btn">
                                Ver Mis Rutas
                            </a>
                        </div>
                        
                        <div class="card" style="background: #d5f4e6; border-left-color: #27ae60;">
                            <p style="margin: 0; color: #27ae60;"><strong>💚 Gracias por tu compromiso con EcoPuntos.</strong> Juntos estamos haciendo la diferencia en la sostenibilidad ambiental.</p>
                        </div>
                    </div>
                    <div class="footer">
                        <p>Este es un correo automático de EcoPuntos. Por favor, no respondas a este mensaje.</p>
                        <p>&copy; 2025 EcoPuntos - Transformando el mundo a través del reciclaje responsable</p>
                    </div>
                </div>
            </body>
            </html>
            ...
            msg = EmailMultiAlternatives(
                subject,
                f"Nueva ruta asignada: {ruta.nombre}",
                settings.DEFAULT_FROM_EMAIL,
                [conductor.email]
            )
            msg.attach_alternative(html_content, "text/html")
            msg.send()
            print(f"✅ Correo de asignación enviado a {conductor.email}")
        except Exception as e:
            print(f"⚠️ Error enviando correo: {e}")
        
        return JsonResponse({
            'success': True,
            'message': f'Ruta "{ruta.nombre}" asignada exitosamente a {conductor.first_name or conductor.username}',
            'ruta_id': ruta.id,
            'conductor_id': conductor.id
        })
        
    except RutaRecoleccion.DoesNotExist:
        return JsonResponse({
            'success': False,
        ...
        return JsonResponse({
            'success': False,
            'message': f'Error al asignar la ruta: {str(e)}'
        }, status=500)