    ...
    try:
        ...
        return JsonResponse({
            ...
            'user': {
                ...
                'zona_asignada': user.zona_asignada if hasattr(user, 'zona_asignada') else None,
            }
        })
...
@require_http_methods(["POST"])
@csrf_exempt
def asignar_zona_conductor(request, user_id):
    """Asignar zona de recolección a un conductor"""
    try:
        conductor = get_object_or_404(Usuario, id=user_id, role='conductor')
        data = json.loads(request.body)
        zona = data.get('zona')
        ...
        # Asignar la zona al conductor
        conductor.zona_asignada = zona
        conductor.save()
        
        # Obtener nombre de la zona para el mensaje
        zona_nombres = dict(Usuario.ZONAS)
        zona_nombre = zona_nombres.get(zona, zona)
        
        return JsonResponse({
            'success': True,
            'message': f'Zona {zona_nombre} asignada correctamente al conductor {conductor.username}.'
        })
